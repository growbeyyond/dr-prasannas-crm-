...
import { Patient, Service, Branch, User, Followup, Priority, Appointment } from '../types';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={resetAndClose}>
      <div className="bg-white w-full max-w-2xl rounded-lg shadow-xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 flex-grow overflow-auto">
            ...
            {selectedPatient && step === 'select_action' && (
                <SelectActionStep 
                    patient={selectedPatient} 
                    onSelectAction={(action) => setStep(action)}
                    onBack={() => { setSelectedPatient(null); setStep('search'); }}
                />
            )}
            {selectedPatient && step === 'followup_form' && (
                <FollowupFormStep 
                    patient={selectedPatient} 
...
const SearchStep = ({ searchTerm, onSearchChange, results, onSelectPatient, onAddNew, isLoading }: any) => (
    <div>
        ...
        <div className="mt-4 max-h-60 overflow-auto">
            ...
            {!isLoading && results.length > 0 && results.map((p: Patient) => (
                <div key={p.id} onClick={() => onSelectPatient(p)} className="p-3 hover:bg-slate-100 cursor-pointer border-b">
            ...
            {!isLoading && searchTerm.length > 1 && results.length === 0 && (
                <div className="text-center p-8 text-slate-500">
                    ...
                    <button onClick={onAddNew} className="mt-2 text-blue-600 font-semibold hover:underline">
                        + Add a New Patient
                    </button>
                </div>
            )}
         ...
         {!isLoading && searchTerm.length > 1 && results.length > 0 && (
             <button onClick={onAddNew} className="mt-4 w-full text-center p-3 bg-slate-100 rounded-md text-blue-600 font-semibold hover:bg-slate-200">
                Can't find them? + Add a New Patient
            </button>
         )}
    </div>
);
        ...
        <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    ...
                    <input name="name" type="text" required className="w-full p-2 border border-slate-300 rounded-md"/>
                </div>
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Phone Number</label>
                    <input name="phone" type="tel" required className="w-full p-2 border border-slate-300 rounded-md"/>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">Date of Birth</label>
                        <input name="dob" type="date" className="w-full p-2 border border-slate-300 rounded-md"/>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">Gender</label>
                        <select name="gender" className="w-full p-2 border border-slate-300 rounded-md">
                ...
                <div className="flex justify-end gap-3 pt-4">
                    ...
                    <button type="submit" disabled={isLoading} className="px-4 py-2 bg-blue-600 text-white rounded-md flex items-center gap-2">
...
const SelectActionStep = ({ patient, onSelectAction, onBack }: any) => (
    <div>
        ...
        <div className="bg-slate-100 p-4 rounded-lg mb-4">
            <p>Selected Patient: <span className="font-bold">{patient.name}</span></p>
            <p className="text-sm text-slate-600">{patient.phone}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <button onClick={() => onSelectAction('followup_form')} className="p-6 bg-blue-50 text-blue-800 rounded-lg text-center hover:bg-blue-100 border border-blue-200">
                <h4 className="font-bold text-lg">Schedule Follow-up</h4>
                <p className="text-sm">Create a reminder for a future check-in.</p>
            </button>
            <button onClick={() => onSelectAction('appointment_form')} className="p-6 bg-green-50 text-green-800 rounded-lg text-center hover:bg-green-100 border border-green-200">
                <h4 className="font-bold text-lg">Book Appointment</h4>
                <p className="text-sm">Schedule a new consultation or service.</p>
            </button>
        </div>
         <div className="mt-6 text-center">
            <button onClick={onBack} className="text-sm text-slate-500 hover:underline">Back to search</button>
        </div>
    </div>
);
         ...
         <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                 <div>
                    ...
                    <input name="scheduled_date" type="date" required defaultValue={new Date().toISOString().split('T')[0]} className="w-full p-2 border border-slate-300 rounded-md"/>
                </div>
                 <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Notes / Reason</label>
                    <textarea name="notes" required className="w-full p-2 border border-slate-300 rounded-md" rows={3}></textarea>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">Priority</label>
                        <select name="priority" defaultValue="normal" className="w-full p-2 border border-slate-300 rounded-md">
                     ...
                     <div>
                        ...
                        <select name="branch_id" defaultValue={currentUser.branch_id || branches[0].id} disabled={currentUser.role !== 'doctor'} className="w-full p-2 border border-slate-300 rounded-md bg-slate-50 disabled:bg-slate-100">
...
const AppointmentFormStep = ({ patient, onSubmit, onBack, isLoading, branches, currentUser, services }: any) => {
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        const formData = new FormData(e.currentTarget);
        const data = Object.fromEntries(formData.entries());
         ...
         <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    ...
                    <select name="service_id" required className="w-full p-2 border border-slate-300 rounded-md">
                        {services.map((s: Service) => <option key={s.id} value={s.id}>{s.name} ({s.duration_minutes} min)</option>)}
                    </select>
                </div>
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Branch</label>
                    <select name="branch_id" defaultValue={currentUser.branch_id || branches[0].id} disabled={currentUser.role !== 'doctor'} className="w-full p-2 border border-slate-300 rounded-md disabled:bg-slate-100">
                        {branches.map((b: Branch) => <option key={b.id} value={b.id}>{b.name}</option>)}
                    </select>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="date" type="date" defaultValue={new Date().toISOString().split('T')[0]} required className="w-full p-2 border border-slate-300 rounded-md"/>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">Time</label>
                        <select name="time" defaultValue="09:00" required className="w-full p-2 border border-slate-300 rounded-md">
